...
(() => {
  ...
  function render() {
    ...
    const views = {
      ...
      live: () => `<div class="card">${toggle("live-enabled", state.live.enabled, "Исправление во время набора", "Обрабатывает текущий набранный фрагмент.")}</div><div class="card"><h2>Как запускать исправление</h2><p>Выберите привычный способ завершить фрагмент.</p><div class="radios"><label><input type="radio" name="trigger" value="DoubleSpace" ${state.live.trigger === "DoubleSpace" ? "checked" : ""}>Двойной пробел</label><label><input type="radio" name="trigger" value="Hotkey" ${state.live.trigger === "Hotkey" ? "checked" : ""}>Горячая клавиша</label></div><div class="field"><label for="live-interval">Интервал между пробелами, мс</label><input id="live-interval" type="number" min="100" max="3000" step="1" value="${state.live.interval}"><p>От 100 до 3000 мс. Меньше значение — быстрее нужно нажать пробел повторно.</p></div><div class="row"><div><h3>Исправить фрагмент</h3></div>${key("liveConvert")}</div><div class="row"><div><h3>Включить или выключить Live</h3></div>${key("liveToggle")}</div></div><div class="card">${toggle("live-hint", state.live.hint, "Подсказка при первом включении", "Показать, как пользоваться Live-режимом.")}</div><div class="hint">Live заменяет текст в активном приложении. Для больших документов удобнее выделение и полная конвертация.</div>`,
  ...
  async function save() {
    ...
    if (page === "live") {
      ...
      command = "saveLive";
      data = { enabled: $("#live-enabled").checked, hint: $("#live-hint").checked, trigger: $("input[name=trigger]:checked").value, interval };
  ...
  if (bridge) request("state").then(result => { state = result; render(); }).catch(error => { $("#content").textContent = error.message; });
